	...
	switch ($type){
		case 'payment':
			$arr = array(
							...
							'ihc_payment_selected' => 'zibal',
						);
		break;
		case 'payment_zibal':
			$arr = array(
							'ihc_zibal_key' => 'zibal',
							'ihc_zibal_direct' => 'disable',
							'ihc_zibal_return_page' => -1,
							'ihc_zibal_status' => 0,
							'ihc_zibal_label' => 'zibal',
							'ihc_zibal_select_order' => 1,
						);
		break;
...
function ihc_insert_update_transaction($u_id, $txn_id, $post_data){
	...
	if ($exists){
		...
		$e = $wpdb->query( 'INSERT INTO '.$table.' VALUES (null, "'.$txn_id.'", '.$u_id.', \''.$post_data.'\', \''.$history_str.'\', null);' );
//		var_dump($e);
//		wp_die();
	}
}
...
function ihc_cancel_level($u_id, $l_id){
	...
	if ($txn_id && $payment_type){
		...
		switch ($payment_type){
			case 'zibal':
				$alias = get_option('ihc_zibal_key');
...
function ihc_check_payment_available($type=''){
	...
	if ($type){
		...
		switch ($type){
			case 'zibal':
				if (!empty($payment_metas['ihc_zibal_key']) && !empty($payment_metas['ihc_zibal_status'])){
					return TRUE;
				}
				break;
			case 'authorize':
