...
function ihc_check_payment_status($p_type){
	...
	switch($p_type){
		case 'zibal':
					  $arr = ihc_return_meta_arr('payment_zibal');
					  if ($arr['ihc_zibal_status'] == 1) { $return['active'] = 'zibal-active'; $return['status'] = 1; }
					  if ($arr['ihc_zibal_key'] != '') $return['settings'] = 'Completed';
					  break;
		case 'bank_transfer':
...
function ihc_check_payment_gateways(){
	...
	if ($levels){
		...
		if ($paid_levels){
			$payments_gateways = array('zibal','bank_transfer');
			$err_msg = TRUE;
